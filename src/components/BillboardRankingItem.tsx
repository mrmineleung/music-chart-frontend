...
const RankingItem = ({ item: props }: RankingItemProps) => {
  const { updateNowPlaying, pendingPlaylist, updatePendingPlaylist } =
    usePlaylist();
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger
                  asChild
                  onClick={() =>
                    handleAddToCurrentPlaylist([
                      ...(pendingPlaylist as RankingItemData[]),
                      props,
                    ])
                  }
                >
      ...
      <div className="grid grid-cols-10 justify-items-center place-items-center md:hidden">
        <div className="col-span-2">
          <div className="flex flex-col">
            ...
            {props.rank_changes_flow == "+" ? (
              <p className="text-sm min-w-8 text-center rounded-sm bg-green-500">
                {props.rank_changes_flow}
                {props.rank_changes_position}
              </p>
            ) : (
              <></>
            )}
            {props.rank_changes_flow == "-" ? (
              <p className="text-sm min-w-8 text-center rounded-sm bg-red-500">
                {props.rank_changes_flow}
                {props.rank_changes_position}
              </p>
            ) : (
            ...
            {props.rank_changes_flow == "NEW" ||
            props.rank_changes_flow == "RE-ENTRY" ? (
              <p className="text-sm min-w-8 max-w-14 text-center rounded-sm bg-yellow-500 px-2">
                {props.rank_changes_flow}
              </p>
            ) : (
              <></>
